...
Берутся товары WB из последнего валидного среза остатков не старше одного дня. Срез фильтруется по `mp='wb'`; нулевой или аномально неполный текущий день заменяется предыдущим валидным днем. Дата выбранного среза указывается в сообщении.

Если валидного среза нет или карточки не сопоставились, workflow падает до отправки. Если данные валидны, но действий нет, сообщение и файл в Пачку не отправляются (`status=skipped`).

### Метрики

| Метрика | Источник | Логика |
|---|---|---|
| FBO | `mp.mp_core__realtime_stocks_data` | последний валидный WB-срез, `SUM(fbo_real)`; fallback не старше одного дня |
