...
- `REPORT_TZ` controls date calculation, default is `Asia/Tbilisi`.
- `REPORT_RUN_LABEL` is shown in the message title, default is `09:20 по Тбилиси`.
...
- The second root message contains the marketer/niche summary and priority queue. Its attached file and thread cover SKU-level problems, actions and controllable optimization methods, including campaign mechanics, prices/SPP, margins, FBO, CRM incoming stock, rating/reviews and fresh positions when available.
...
The daily schedule is owned by Cloudflare Worker `cloudflare/abcage_notification`, which calls GitHub's workflow dispatch API every day at 09:20 Tbilisi time (`05:20 UTC`).
